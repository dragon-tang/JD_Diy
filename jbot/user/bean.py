...
@user.on(events.NewMessage(pattern=r'^-b', outgoing=True))
async def bot_bean(event):
    ...
    if text==None:
        await event.edit('请指定要查询的账号,格式: -b 1')
        return    
    else:
        notification = await event.edit('开始查询账号'+text+'的资产，请稍后...')
    
    if V4 and text == 'in':
        subprocess.check_output(
            'jcsv', shell=True, stderr=subprocess.STDOUT)
        creat_bean_counts(BEAN_IN_FILE)
        msg = await user.send_message(event.chat_id, '您的近日收入情况', file=BEAN_IMG)
